...
## 2100. [<font color=yellow>适合打劫银行的日子</font>](https://leetcode-cn.com/problems/find-good-days-to-rob-the-bank/)
...
<div><div>Related Topics</div><div><li>数组</li><li>动态规划</li><li>前缀和</li></div>
...
class Solution {
    public List<Integer> goodDaysToRobBank(int[] security, int time) {
        ...
        int[] dpBefore = new int[length];
        int[] dpAfter = new int[length];

        for (int i = 1; i < length; i++) {
            if (security[i] <= security[i - 1]) {
                dpBefore[i] = dpBefore[i - 1] + 1;
            }
            if (security[i] >= security[i - 1]) {
                dpAfter[i] = dpAfter[i - 1] + 1;
            }
        }

        for (int i = time; i < length - time; i++) {
            if (dpBefore[i] >= time && dpAfter[i + time] >= time) {
                result.add(i);
            }
        }
